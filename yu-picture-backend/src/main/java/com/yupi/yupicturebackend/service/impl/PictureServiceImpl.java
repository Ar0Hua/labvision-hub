...
//            }
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(id == null || reviewStatusEnum == null || PictureReviewStatusEnum.REVIEWING.equals(reviewStatusEnum), ErrorCode.PARAMS_ERROR, "参数错误");
        // 判断图片是否存在
        Picture oldPicture = this.getById(id);
        ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR, "图片不存在");
        // 校验审核状态是否重复
        if (oldPicture.getReviewStatus() == reviewStatusEnum.getValue()) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "审核状态不能重复");
        }
        // 数据库操作
